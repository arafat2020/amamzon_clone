import React, { useContext } from 'react';
import './Header.css';
import SearchIcon from '@material-ui/icons/Search';
import ShoppingBasketIcon from '@material-ui/icons/ShoppingBasket';
import { Link } from 'react-router-dom';
import { UserContext } from './Op-stateprovider';
import { auth } from './firebase';

const Header = () => {
    const user = useContext(UserContext)
    // console.log(user.location.length)
    const authHandeler = () => {
        if (user.name) {
            auth.signOut()
        }
    }
    
    return (
        <div className="header">
            <Link to="/">
                <img
                className="header_logo"
                src="http://pngimg.com/uploads/amazon/amazon_PNG11.png"
                alt="logo"
               />
            </Link>
            <div className="header_search">
                <input
                    className="header_searchIn"
                    type="text"
                />
                <SearchIcon
                    className="search_icon"
                />
            </div>
            <div className="header_nav">
                <Link to={!user.name && '/login'}>
                <div onClick={authHandeler} className="header_option">
                    <span className="header_optionLineOne">
                        Hello {user.name? user.name.email:'Guist'}
                    </span>
                    <span className="header_optionLineTwo">
                            {user.name ? 'Sign Out' : 'Sign In'}
                    </span>
                </div>
                </Link>
                <Link to='/order'>
                <div className="header_option">
                    <span className="header_optionLineOne">
                        Returns
                    </span>
                    <span className="header_optionLineTwo">
                       & Orders
                    </span>
                </div>
                </Link>
                <div className="header_option">
                    <span className="header_optionLineOne">
                        Your
                    </span>
                    <span className="header_optionLineTwo">
                        Prime
                    </span>
                </div>
                <Link to="/checkout">
                <div className="shopingBasket">
                    <ShoppingBasketIcon />
                    <span
                       className="header_optionLineTwo busket_count" 
                    >
                        {user.count.basket.length}
                    </span>
                </div>
                </Link>
            </div>
        </div>
    );
};

export default Header;